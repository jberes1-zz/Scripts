...
while (KeywordIterator.hasNext()) {
 ...
 var RepIterator = AdWordsApp.report("SELECT CampaignName, AdGroupName, HeadlinePart1, HeadlinePart2, Description, Path1, Path2, Labels, LabelIds FROM AD_PERFORMANCE_REPORT WHERE CampaignStatus = ENABLED AND LabelIds CONTAINS_ANY ['1441044526'] during LAST_14_DAYS");
 var KeywordIterator = RepIterator.rows();
 while (KeywordIterator.hasNext()) {
   var KWRow = KeywordIterator.next();
   var Campaign = KWRow["CampaignName"];

...
var SPREADSHEET_URL = "https://docs.google.com/a/medicuja.com/spreadsheets/d/1h5A7dq1O1kPZDDVZ-XRuJ7y060ZKlx72lBcW7mAn79Y/edit?usp=sharing"
...
//Write data into Ads Sheet
  var RepIterator = AdWordsApp.report("SELECT CampaignName, AdGroupName, HeadlinePart1, HeadlinePart2, Description, Path1, Path2, Labels, Clicks, Impressions, Ctr, AverageCpc, Cost, AveragePosition, Conversions, CostPerConversion, ConversionRate FROM AD_PERFORMANCE_REPORT WHERE CampaignStatus = ENABLED AND Impressions > 50 during LAST_14_DAYS");
...
var RepIterator = AdWordsApp.report("SELECT CampaignName, AdGroupName, HeadlinePart1, HeadlinePart2, Description, Path1, Path2, Labels, LabelIds FROM AD_PERFORMANCE_REPORT WHERE CampaignStatus = ENABLED AND LabelIds CONTAINS_ANY ['1440788072'] during LAST_14_DAYS");
var KeywordIterator = RepIterator.rows();
while (KeywordIterator.hasNext()) {
  var KWRow = KeywordIterator.next();
  var Campaign = KWRow["CampaignName"];

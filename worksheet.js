...
* Updated by Wesley Parker - www.clicteq.com
**********************************************/
var EXTRA_LOGS = true;
var TO = ['jbs@medicuja.com'];
...
//will be ignored.
var OVERRIDE_LAST_TOUCHED_DATE = 'Jul 1, 2017';
...
function main() {
  ...
  while(agIter.hasNext()) {
    ...
    for(var loopNum = 0; loopNum < numLoops; loopNum++) {
      ...
      var adSelector = ag.ads().withCondition('Status = ENABLED').withCondition('AdType = EXPANDED_TEXT_AD');
...
function checkForPreviousWinner(ag,isMobile) {
  var adSelector = ag.ads().withCondition('Status = ENABLED')
                           .withCondition('AdType = EXPANDED_TEXT_AD');
  if(!AdWordsApp.getExecutionInfo().isPreview()) {
